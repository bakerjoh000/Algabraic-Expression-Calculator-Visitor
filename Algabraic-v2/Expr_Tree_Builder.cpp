...
void Expr_Tree_Builder::build_addition_operand()
{
	...
	checkWeight(this->operators, this->numbers, this->tree_, node);
...
void Expr_Tree_Builder::build_multiplication_operand()
{

	Expr_Node * node = new Multiplication_Node();
	checkWeight(this->operators, this->numbers, this->tree_, node);

}

void Expr_Tree_Builder::build_division_operand()
{

	Expr_Node * node = new Division_Node();
	checkWeight(this->operators, this->numbers, this->tree_, node);
...
void Expr_Tree_Builder::checkWeight(Stack<Expr_Node *>& operators, Stack<Expr_Node *>& numbers, Expr_Node * rootNode, Expr_Node * expr_node)
{

	//move the operators nodes(and their children) according to their weight
	...
	else
	{
		...
		if (tempnode->weight() < expr_node->weight())
		{
			operators.push(expr_node);
		...
		else
		{
			...
			while (expr_node->weight() <= tempnode->weight() && operators.size() != 0)
			{
